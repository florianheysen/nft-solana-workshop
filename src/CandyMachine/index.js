...
const CandyMachine = ({ walletAddress }) => {
  ...
  const renderMintedItems = () => {
    return(
      ...
      <p className="sub-text">Vos acquisitions ✨</p>
  ...
  return (
    machineStats && (
      <div className="machine-container">
        {renderDropTimer()}
        <p>{`Oeuvres acquises: ${machineStats.itemsRedeemed} / ${machineStats.itemsAvailable}`}</p>
          ...
          {machineStats.itemsRedeemed === machineStats.itemsAvailable ? (
            ...
            <button
              ...
              disabled={isMinting}
            >
              {isMinting ? 'Acquisition en cours...' : 'Acquérir une oeuvre'}
            </button>
          )}
        {mints.length > 0 && renderMintedItems()}
        {isLoadingMints && <p>Chargement de vos acquisitions...</p>}
      </div>
    )
  );
};
